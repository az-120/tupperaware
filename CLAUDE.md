...
Task: Fix calendar date picker visibility on white background.

### Problem

The date picker (DateTimePicker) in app/item/add.tsx and
app/item/edit.tsx is invisible/hard to see because it inherits
the white background.

### Fix

In both `app/item/add.tsx` and `app/item/edit.tsx`:

Wrap the DateTimePicker in a container with a subtle background:
...
```typescript
datePickerContainer: {
  backgroundColor: Colors.surfaceAlt,  // subtle off-white #F8F8F8
  borderRadius: 14,
  borderWidth: 0.5,
...
- Only change the date picker container styling
- Do not touch any other styles or logic
